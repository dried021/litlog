...
import { validateNameFormat, validateEmailFormat } from './validation';
...
export function useSignUpSubmit({
  ...
  const handleSubmit = async (e) => {
    ...
    const { valid: emailValid, message: emailMessage } = validateEmailFormat(email);
    if (!emailValid) return alert(emailMessage);
    ...
    try {
      ...
      await axios.post("http://localhost:9090/api/sign-up", dto);
      alert("회원가입 성공!");
    } catch (err) {
